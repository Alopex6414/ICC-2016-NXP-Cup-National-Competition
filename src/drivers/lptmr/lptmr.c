...
void lptmr_counter_init(LPT0_ALTn altn, u16 count, u8 PrescaleValue, LPT_CFG cfg)
{
    ...
    LPTMR0_PSR=0x00;                        //添加预分频寄存器
    LPTMR0_CMR=0x00;                        //添加比较寄存器
    /* 设置累加计数值  */
...
u16 get_counter_value(void) //读脉冲计数值
{
     LPTMR0_CNR=0x1;
     return LPTMR0_CNR;
}
void lptmr_clear_registers(void)//寄存器清0
{
     LPTMR0_CSR=0x00;
     LPTMR0_PSR=0x00;
     LPTMR0_CMR=0x00;
}
void lptmr_pulse_counter(char pin_select)//选取通道脉冲计数。ALT1：A19.ALT2：C5.ALT3：.
{
     unsigned int compare_value=5000;
  ...
  else
  {
    ...
    LPTMR0_CMR=LPTMR_CMR_COMPARE(compare_value);  //Set compare value,大于此值计数清0
