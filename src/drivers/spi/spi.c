...
void spi_init(SPIn spin,SPI_CFG master)
{
    ...
    else if(spin == 1)
    {
       ...
       // PORTE_PCR3 = 0 | PORT_PCR_MUX(0x2);                     //SIN
        PORTE_PCR0 = 0 | PORT_PCR_MUX(0x2) | PORT_PCR_DSE_MASK; //PCS1
    }
    else 
    {   
    ...
    SPI_MCR_REG(SPIN[spin]) &= ~SPI_MCR_MDIS_MASK;
    ...
    if(master == MASTER)
    {
        ...
        SPI_CTAR_REG(SPIN[spin],0) = (0
                        ...
                        | SPI_CTAR_PBR(0)            //波特率分频器 ，0~3 对应的分频值Prescaler为 2、3、5、7
...
**********************************************************************************/
void SPI_Send(SPIn spin,u8 data)
{
    SPI_SR_REG(SPIN[spin]) |= SPI_SR_EOQF_MASK;
	SPI_MCR_REG(SPIN[spin]) &= (~SPI_MCR_HALT_MASK) & (~SPI_MCR_MDIS_MASK);   /* 进入RUNNING状态 */
    
    SPI_PUSHR_REG(SPIN[spin]) = SPI_PUSHR_CTAS(0) | SPI_PUSHR_EOQ_MASK | SPI_PUSHR_CTCNT_MASK | SPI_PUSHR_PCS(2) | SPI_PUSHR_TXDATA(data);
                                                                              /* 写入数据并清空计数寄存器 */ 
                                                                              /* 置位EOQ表示该数据是队列中最后一个,即使能发送 */
	while((SPI_SR_REG(SPIN[spin]) & SPI_SR_TCF_MASK) == 0);	
	SPI_SR_REG(SPIN[spin]) |= SPI_SR_TCF_MASK;
     
    SPI_MCR_REG(SPIN[spin]) |= SPI_MCR_CLR_TXF_MASK | SPI_MCR_CLR_RXF_MASK;   /* 清空队列(这句很重要) */
...
u32 spi_re_max(SPIn spin,u8 data[],u32 maxlen)
{
    ...
    while(SPI_SR_REG(SPIN[spin]) & SPI_SR_RFDF_MASK)    //RFDF为1，Rx FIFO is not empty.
    {
        data[n++] = (u16)SPI_POPR_REG(SPIN[spin]);    //保存接收到的数据            
        SPI_SR_REG(SPIN[spin]) |= SPI_SR_RFDF_MASK;
        if(n>=maxlen)
        {
            break;
