   ...
   // ASSERT( ((adcn == ADC0) && (ch >= AD8 && ch <= AD18)) || ((adcn == ADC1) && (ch >= AD4a && ch <= AD17)) ) ; //使用断言检测ADCn_CHn是否正常
    ...
    case ADC0:       /*   ADC0  */
        ...
        case AD18:   //ADC0_SE17 -- PTE25
            ...
            break;
        case AD19:
            Master_Adc_Config.STATUS1A = AIEN_ON | DIFF_SINGLE | ADC_SC1_ADCH(DM0)/* | AVGE_ENABLED | ADC_SC3_AVGS(AVGS_16)*/;
            adc_config_alt(ADC0_BASE_PTR, &Master_Adc_Config);  //0M0// 配置 ADC0
            break;
        case AD20:
            Master_Adc_Config.STATUS1A = AIEN_ON | DIFF_SINGLE | ADC_SC1_ADCH(DM1)/* | AVGE_ENABLED | ADC_SC3_AVGS(AVGS_16)*/;
            adc_config_alt(ADC0_BASE_PTR, &Master_Adc_Config);  //0M1// 配置 ADC0
            break;
        default:
            return;
        }
    ...
    case ADC1:       /*   ADC1    */
        ...
        case AD19:
            Master_Adc_Config.STATUS1A = AIEN_ON | DIFF_SINGLE | ADC_SC1_ADCH(DM0)/* | AVGE_ENABLED | ADC_SC3_AVGS(AVGS_16)*/;
            adc_config_alt(ADC0_BASE_PTR, &Master_Adc_Config);  //1M0// 配置 ADC1
            break;
        case AD20:
            Master_Adc_Config.STATUS1A = AIEN_ON | DIFF_SINGLE | ADC_SC1_ADCH(DM1)/* | AVGE_ENABLED | ADC_SC3_AVGS(AVGS_16)*/;
            adc_config_alt(ADC0_BASE_PTR, &Master_Adc_Config);  //1M1// 配置 ADC1
            break;
        default:
            break;
        }
...
*  备    注：修改苏州大学的例程
*************************************************************************/
void GetAD(float *Angle_AD,float *AngleSpeed_AD)
{
     int i=0;
     u32 angle_sum=0;                                                                
     u32 angle_dot_sum=0; 
     ...
     for(i=0;i<20;i++) 
     {
        adc_start(ADC0, AD19, ADC_12bit);	  //启动ADC转换
        adc_start(ADC1, AD19, ADC_12bit);	  //启动ADC转换

         while (( ADC_SC1_REG(ADCx[ADC0], 0 ) & ADC_SC1_COCO_MASK ) != ADC_SC1_COCO_MASK);
         angle_sum += ADC_R_REG(ADCx[ADC0], 0);
         ADC_SC1_REG(ADCx[ADC0], 0) &= ~ADC_SC1_COCO_MASK;
         
         while (( ADC_SC1_REG(ADCx[ADC1], 0 ) & ADC_SC1_COCO_MASK ) != ADC_SC1_COCO_MASK);
         angle_dot_sum += ADC_R_REG(ADCx[ADC1], 0);
         ADC_SC1_REG(ADCx[ADC1], 0) &= ~ADC_SC1_COCO_MASK;  
     }
      *Angle_AD=angle_sum*0.05;
      *AngleSpeed_AD=angle_dot_sum*0.05;
}
