...
void gpio_init (PORTx portx, u8 n, GPIO_CFG cfg, u8 data)
{
    ...
    if(cfg == GPI_DOWN || cfg == GPI_DOWN_PF)//输入下拉
    {
      PORT_PCR_REG(PORTX[portx], n) = (0x0002 | PORT_PCR_MUX(1) | cfg);
    }
    
    else
    {
      PORT_PCR_REG(PORTX[portx], n) = (0x0003 | PORT_PCR_MUX(1) | cfg);
    }
...
{
    return ((GPIO_PDIR_REG(GPIOx[(portx)])>>(n))&0x1);
}
